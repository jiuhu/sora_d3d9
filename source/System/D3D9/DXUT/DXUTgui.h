...
public:
    ...
    void                Init( CDXUTDialogResourceManager* pManager, bool bRegisterDialog,
                              CHAR * pszControlTextureFilename );
    void                Init( CDXUTDialogResourceManager* pManager, bool bRegisterDialog,
    ...
    // Control creation
    HRESULT             AddStatic( int ID, CHAR * strText, int x, int y, int width, int height, bool bIsDefault=false,
                                   CDXUTStatic** ppCreated=NULL );
    HRESULT             AddButton( int ID, CHAR * strText, int x, int y, int width, int height, UINT nHotkey=0,
                                   bool bIsDefault=false, CDXUTButton** ppCreated=NULL );
    HRESULT             AddCheckBox( int ID, CHAR * strText, int x, int y, int width, int height, bool bChecked=false,
                                     UINT nHotkey=0, bool bIsDefault=false, CDXUTCheckBox** ppCreated=NULL );
    HRESULT             AddRadioButton( int ID, UINT nButtonGroup, CHAR * strText, int x, int y, int width,
    ...
    HRESULT             AddEditBox( int ID, CHAR * strText, int x, int y, int width, int height, bool bIsDefault=
	...
	inline HRESULT CDXUTDialog::DrawRect( RECT* pRect, D3DCOLOR color )
	{
		//if( m_pManager->GetD3D9Device() )
			return DrawRect9( pRect, color );
	}
    HRESULT             DrawRect9( RECT* pRect, D3DCOLOR color );
	HRESULT             DrawPolyLine(D3DXVECTOR2* apPoints, UINT nNumPoints, D3DCOLOR color);
    
	inline HRESULT CDXUTDialog::DrawSprite( CDXUTElement* pElement, RECT* prcDest, float fDepth )
	{
		//if( m_pManager->GetD3D9Device() )
			return DrawSprite9( pElement, prcDest );
	}
    HRESULT             DrawSprite9( CDXUTElement* pElement, RECT* prcDest );
    HRESULT             CalcTextRect( CHAR * strText, CDXUTElement* pElement, RECT* prcDest, int nCount = -1 );
    
	inline HRESULT CDXUTDialog::DrawText( CHAR * strText, CDXUTElement* pElement, RECT* prcDest, bool bShadow = false, int nCount = -1 )
	{
		//if( m_pManager->GetD3D9Device() )
			return DrawText9( strText, pElement, prcDest, bShadow, nCount );
	}
    HRESULT             DrawText9( CHAR * strText, CDXUTElement* pElement, RECT* prcDest, bool bShadow = false,
                                   int nCount = -1 );
    ...
    void                Refresh();
    inline HRESULT CDXUTDialog::OnRender( float fElapsedTime )
	{
		//if( m_pManager->GetD3D9Device() )
    ...
    // all the controls.
    HRESULT             SetFont( UINT index, CHAR * strFaceName, LONG height, LONG weight );
    DXUTFontNode* GetFont( UINT index );

    HRESULT             SetTexture( UINT index, CHAR * strFilename );
    HRESULT             SetTexture( UINT index, CHAR * strResourceName, HMODULE hResourceModule );
...
public:
    ...
    // D3D9 specific
	HRESULT OnD3D9CreateDevice(LPDIRECT3DDEVICE9 pd3dDevice, LPD3DXSPRITE pSprite);
    ...
    int     AddFont( CHAR * strFaceName, LONG height, LONG weight );
    int     AddTexture( CHAR * strFilename );
    int     AddTexture( CHAR * strResourceName, HMODULE hResourceModule );
...
public:
    ...
    HRESULT         GetTextCopy( __out_ecount(bufferCount) CHAR * strDest, 
                                 UINT bufferCount );
    CHAR*         GetText()
    {
        return m_strText;
    }
    HRESULT         SetText( CHAR * strText );
...
public:
    ...
    bool                    InsertChar( size_t nIndex, CHAR CHAR ); // Inserts the char at specified index. If nIndex == -1, insert to the end.
    bool                    RemoveChar( size_t nIndex );  // Removes the char at specified index. If nIndex == -1, remove the last char.
    bool                    InsertString( size_t nIndex, const CHAR* pStr, int nCount = -1 );  // Inserts the first nCount characters of the string pStr at specified index.  If nCount == -1, the entire string is inserted. If nIndex == -1, insert to the end.
    bool                    SetText( CHAR * wszText );
...
public:
    ...
    void            SetText( CHAR * wszText, bool bSelected = false );
    const CHAR *	GetText()
    {
