...
DWORD                   DXUT_LINE_VERTEX::FVF = D3DFVF_XYZRHW | D3DFVF_DIFFUSE;
...
//--------------------------------------------------------------------------------------
void CDXUTDialog::Init( CDXUTDialogResourceManager* pManager, bool bRegisterDialog, CHAR * pszControlTextureFilename )
{
    m_pManager = pManager;
    if( bRegisterDialog )
        pManager->RegisterDialog( this );
...
//--------------------------------------------------------------------------------------
void CDXUTDialog::Init( CDXUTDialogResourceManager* pManager, bool bRegisterDialog,
                        CHAR * szControlTextureResourceName, HMODULE hControlTextureResourceModule )
{
    m_pManager = pManager;
    if( bRegisterDialog )
        pManager->RegisterDialog( this );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialogResourceManager::OnD3D9CreateDevice(LPDIRECT3DDEVICE9 pd3dDevice, LPD3DXSPRITE pSprite)
{
    HRESULT hr = S_OK;
    int i = 0;
	...
	else
	{
		...
		if (FAILED(hr))
			return hr;//DXUT_ERR( "D3DXCreateSprite", hr );
	}
    return S_OK;
}
...
HRESULT CDXUTDialog::OnRender9( float fElapsedTime )
{
    ...
    assert( m_pManager->GetD3D9Device() && m_pManager->m_pStateBlock &&
            "To fix hook up CDXUTDialogResourceManager to device callbacks.  See comments for details" );
...
//--------------------------------------------------------------------------------------
int CDXUTDialogResourceManager::AddFont( CHAR * strFaceName, LONG height, LONG weight )
{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::SetFont( UINT index, CHAR * strFaceName, LONG height, LONG weight )
{
    // If this assert triggers, you need to call CDXUTDialog::Init() first.  This change
    // was made so that the DXUT's GUI could become seperate and optional from DXUT's core.  The 
    // creation and interfacing with CDXUTDialogResourceManager is now the responsibility 
...
//--------------------------------------------------------------------------------------
int CDXUTDialogResourceManager::AddTexture( CHAR * strFilename )
{
...
//--------------------------------------------------------------------------------------
int CDXUTDialogResourceManager::AddTexture( CHAR * strResourceName, HMODULE hResourceModule )
{
    // See if this texture already exists
    for( int i = 0; i < m_TextureCache.GetSize(); i++ )
    {
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::SetTexture( UINT index, CHAR * strFilename )
{
    // If this assert triggers, you need to call CDXUTDialog::Init() first.  This change
    // was made so that the DXUT's GUI could become seperate and optional from DXUT's core.  The 
    // creation and interfacing with CDXUTDialogResourceManager is now the responsibility 
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::SetTexture( UINT index, CHAR * strResourceName, HMODULE hResourceModule )
{
    // If this assert triggers, you need to call CDXUTDialog::Init() first.  This change
    // was made so that the DXUT's GUI could become seperate and optional from DXUT's core.  The 
    // creation and interfacing with CDXUTDialogResourceManager is now the responsibility 
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddStatic( int ID, CHAR * strText, int x, int y, int width, int height, bool bIsDefault,
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddButton( int ID, CHAR * strText, int x, int y, int width, int height, UINT nHotkey,
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddCheckBox( int ID, CHAR * strText, int x, int y, int width, int height, bool bChecked,
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddRadioButton( int ID, UINT nButtonGroup, CHAR * strText, int x, int y, int width, int height,
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddEditBox( int ID, CHAR * strText, int x, int y, int width, int height, bool bIsDefault,
...
HRESULT CDXUTDialog::AddControl( CDXUTControl* pControl )
{
    ...
    if( FAILED( hr ) )
        return hr;//DXTRACE_ERR( "CDXUTDialog::InitControl", hr );

    // Add to the list
    hr = m_Controls.Add( pControl );
    if( FAILED( hr ) )
    {
        return hr;//DXTRACE_ERR( "CGrowableArray::Add", hr );
    }
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::DrawPolyLine( D3DXVECTOR2* apPoints, UINT nNumPoints, D3DCOLOR color )
{
	DXUT_LINE_VERTEX* vertices = new DXUT_LINE_VERTEX[nNumPoints];
    if( vertices == NULL )
        return E_OUTOFMEMORY;

	DXUT_LINE_VERTEX* pVertex = vertices;
	D3DXVECTOR2* pt = apPoints;
    ...
    IDirect3DVertexDeclaration9* pDecl = NULL;
    pd3dDevice->GetVertexDeclaration( &pDecl );  // Preserve the sprite's current vertex decl
	pd3dDevice->SetFVF(DXUT_LINE_VERTEX::FVF);

    pd3dDevice->SetTextureStageState( 0, D3DTSS_COLOROP, D3DTOP_SELECTARG2 );
    pd3dDevice->SetTextureStageState( 0, D3DTSS_ALPHAOP, D3DTOP_SELECTARG2 );

	pd3dDevice->DrawPrimitiveUP(D3DPT_LINESTRIP, nNumPoints - 1, vertices, sizeof(DXUT_LINE_VERTEX));
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::CalcTextRect( CHAR * strText, CDXUTElement* pElement, RECT* prcDest, int nCount )
{
    HRESULT hr = S_OK;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::DrawText9( CHAR * strText, CDXUTElement* pElement, RECT* prcDest, bool bShadow, int nCount )
{
    HRESULT hr = S_OK;
...
HRESULT CDXUTDialogResourceManager::CreateTexture9( UINT iTexture )
{
    ...
    if( !pTextureNode->bFileSource )
    {
        if( pTextureNode->nResourceID == 0xFFFF && pTextureNode->hResourceModule == ( HMODULE )0xFFFF )
        {
            ...
            if( FAILED( hr ) )
                return hr;//DXTRACE_ERR( "D3DXCreateTextureFromFileInMemoryEx", hr );
            //DXUT_SetDebugName( pTextureNode->pTexture9, "DXUT GUI Texture" );
        }
        else
        {
            CHAR * pID = pTextureNode->nResourceID ? ( CHAR * )( size_t )pTextureNode->nResourceID :
            ...
            if( FAILED( hr ) )
                return hr;//DXTRACE_ERR( "D3DXCreateTextureFromResourceEx", hr );
        }
    }
    else
    {
        ...
        if( FAILED( hr ) )
        {
            return hr;//DXTRACE_ERR( "D3DXCreateTextureFromFileEx", hr );
        }
    }
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTStatic::GetTextCopy( __out_ecount(bufferCount) CHAR * strDest, 
                                  UINT bufferCount )
{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTStatic::SetText( CHAR * strText )
{
    if( strText == NULL )
    {
        m_strText[0] = 0;
...
HRESULT CDXUTComboBox::AddItem( const CHAR* strText, void* pData )
{
    ...
    if( pItem == NULL )
    {
        return S_FALSE;//DXTRACE_ERR_MSGBOX( "new", E_OUTOFMEMORY );
    }
        ...
        case WM_LBUTTONDBLCLK:
            {
                ...
                if( PtInRect( &m_rcUpButton, pt ) )
                {
                    ...
                    UpdateThumbRect();
                    m_Arrow = CLICKED_UP;
                    m_dArrowTS = DXUTGetGlobalTimer()->GetTime();
                    return true;
                }
...
void CDXUTScrollBar::Render( float fElapsedTime )
{
    ...
    if( m_Arrow != CLEAR )
    {
		double dCurrTime = DXUTGetGlobalTimer()->GetTime();
...
void CDXUTListBox::Render( float fElapsedTime )
{
    ...
    if( m_Items.GetSize() > 0 )
    {
        ...
        for( int i = m_ScrollBar.GetTrackPos(); i < ( int )m_Items.GetSize(); ++i )
        {
            ...
            // Determine if we need to draw this item with the
            // selected element.
...
bool                    CDXUTEditBox::s_bHideCaret;   // If true, we don't draw the caret.
...
//--------------------------------------------------------------------------------------
void CDXUTEditBox::SetText( CHAR * wszText, bool bSelected )
{
    assert( wszText != NULL );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTEditBox::GetTextCopy( __out_ecount(bufferCount) CHAR * strDest, 
                                   UINT bufferCount )
{
    assert( strDest );
...
HRESULT CUniBuffer::Analyse()
{
    ...
    HDC hDC = m_pFontNode->pFont9->GetDC();
    HRESULT hr = _ScriptStringAnalyse( hDC,
                                       m_pwszBuffer,
                                       strlen( m_pwszBuffer ) + 1,  // NULL is also analyzed.
                                       strlen( m_pwszBuffer ) * 3 / 2 + 16,
                                       -1,
...
//--------------------------------------------------------------------------------------
bool CUniBuffer::InsertChar( size_t nIndex, CHAR wChar )
{
    assert( nIndex >= 0 );

    if( nIndex < 0 || nIndex > strlen( m_pwszBuffer ) )
    ...
    if( (int)(strlen( m_pwszBuffer ) + 1) >= m_nBufferSize )
    {
        if( !SetBufferSize( -1 ) )
    ...
    CHAR* dest = m_pwszBuffer + strlen( m_pwszBuffer ) + 1;
...
//--------------------------------------------------------------------------------------
bool CUniBuffer::RemoveChar( size_t nIndex )
{
    if( !strlen( m_pwszBuffer ) || nIndex < 0 || nIndex >= strlen( m_pwszBuffer ) )
        return false;  // Invalid index

    MoveMemory( m_pwszBuffer + nIndex, m_pwszBuffer + nIndex + 1, sizeof( CHAR ) *
                ( strlen( m_pwszBuffer ) - nIndex ) );
    m_bAnalyseRequired = true;
    return true;
}
...
// If nIndex == -1, insert to the end.
//--------------------------------------------------------------------------------------
bool CUniBuffer::InsertString( size_t nIndex, const CHAR* pStr, int nCount )
{
    assert( nIndex >= 0 );
    if( nIndex < 0 )
        return false;

    if( nIndex > strlen( m_pwszBuffer ) )
        return false;  // invalid index
    ...
    if( (int)(strlen( m_pwszBuffer ) + nCount) >= m_nBufferSize )
    {
        if( !SetBufferSize( strlen( m_pwszBuffer ) + nCount + 1 ) )
            return false;  // out of memory
    }

    MoveMemory( m_pwszBuffer + nIndex + nCount, m_pwszBuffer + nIndex, sizeof( CHAR ) *
                ( strlen( m_pwszBuffer ) - nIndex + 1 ) );
...
//--------------------------------------------------------------------------------------
bool CUniBuffer::SetText( CHAR * wszText )
{
    assert( wszText != NULL );
...
HRESULT CUniBuffer::XtoCP( int nX, int* pCP, int* pnTrail )
{
    ...
    *pCP = 0; *pnTrail = FALSE;  // Default
    ...
    else if( *pCP > (int)strlen( m_pwszBuffer ) && *pnTrail == FALSE )
    {
        *pCP = strlen( m_pwszBuffer ); *pnTrail = TRUE;
    }
